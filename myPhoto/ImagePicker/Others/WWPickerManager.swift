...
public class WWPickerManager: NSObject {
    ...
    class public var pickerPhotoSize: CGSize {
        assert(WWImagePickerOptions.photoLineNum > 2, "列数最小为2")
        ...
        let width = (screenWidth - CGFloat(WWImagePickerOptions.photoInterval) * (CGFloat(WWImagePickerOptions.photoLineNum) - 1)) / CGFloat(WWImagePickerOptions.photoLineNum)
    ...
    public func getAblumTitle(_ ablumIndex: Int) -> String {
        if let ablum = self.photoAlbums[WW_safe: ablumIndex] {
    ...
    func remove(formSelect alasset: PHAsset) {
        if let index = index(ofSelect: alasset) {
            selectPhotoArr.WW_removeSafe(at: index)
        }
    }
        ...
        if photoAsset != nil {
            ...
            self.photoManage.requestImage(for: photoAsset!, targetSize: photoScaleSize, contentMode: .aspectFill, options: self.photoOption, resultHandler: { (image, infoDic) in
                if image != nil {
                    ...
                    let image_buffer = WWImagePickerBundle.imageFromBundle("image-buffer")
    ...
    public func getPhotoDefalutSize(_ ablumIndex: Int, photoIndex: Int, resultHandler: ((UIImage?, [AnyHashable: Any]?) -> Void)?) {
        getPhoto(ablumIndex, photoIndex: photoIndex, photoSize: WWPickerManager.pickerPhotoSize, resultHandler: resultHandler)
    }
    ...
    private func getPhotoAlbum() {
        ...
        //将获取的相册加入到相册的数组中
        photoAlbums.append([WWImagePickerBundle.localizedString(key: "全部照片"): allResult])
